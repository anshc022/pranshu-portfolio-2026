...
const BentoCard = ({ children, className, delay = 0 }) => (
  <Tilt options={defaultTiltOptions} className={`h-full ${className}`}>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: "-50px" }}
        transition={{ duration: 0.5, delay }}
        className={`glass-card rounded-3xl p-6 md:p-8 flex flex-col justify-between overflow-hidden relative group h-full`}
      >
